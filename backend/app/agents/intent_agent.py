...
class IntentAnalysisAgent:
    ...
    async def analyze(self, context: UserContext) -> IntentAnalysis:
        ...
        分析用户意图
        """
        system_prompt = """你是一个专业的意图分析助手。请分析用户的输入，并返回以下格式的 JSON 响应：
{
    "intent": "用户的主要意图（例如：查询余额、转账、咨询产品等）",
...
请确保：
1. intent 应该是具体的业务意图
2. confidence 应该是 0-1 之间的浮点数
3. entities 应该包含从用户输入中提取的关键信息
4. 返回的必须是合法的 JSON 格式"""
